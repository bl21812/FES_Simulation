...
def plotOutput(time, thetas, torques, labels, colours, dir, fileName, muscleNormLengths = None, fes = None):
  ...
  @param fes: function for the fes signal or a PID FES object.
  ...
  if plotMuscleNormLengths:
    ...
    if callable(fes):
      ...
      fesOutput = fesOutput * 1000 # convert to mV
    else:
      fesOutput = np.array(fes.emgPoints) * 1000
      indices = [fes.times.index(t) for t in time]
      fesOutput = fesOutput[indices]

    axs[2].set(xlabel = "Time (s)", ylabel = "FES sEMG Signal(mV)")
